...
async function main() {
  program
    ...
    .option("--no-git", "Skip Git operations (commit)")
  ...
  try {
    ...
    // Commit changes if Git is available and not disabled
    if (!options.noGit && isGitRepo()) {
      try {
    ...
    if (!options.noGit && isGitRepo()) {
      console.log("Ready to publish! The version changes have been committed.");
    } else {
      console.log(
